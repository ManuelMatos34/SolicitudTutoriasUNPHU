    ...
    public class AdminTutoriasController : Controller
    {
        ...
        public List<Tutorias> tutorias(string filtro, string buscar)
        {
            ...
            else if (buscar != null)
            {
                SqlConnection conexion = Conexion.GetConexion();
                conexion.Open();
                string sql = "  select e.id, concat(f.nombre,' ',f.apellido) as nombre_estudiante, e.asignatura, a.nombre_asignatura, concat(b.nombre,' ',b.apellido) as nombre_profesor, e.horario, e.estatus\r\n  from asignaturas a, profesores b, tutoria e, usuarios f\r\n  where e.id_profesor = b.matricula and e.id_usuario = f.matricula and e.asignatura = a.codigo_asignatura and a.nombre_asignatura like '%"+buscar+"%'";
                SqlCommand comando = new SqlCommand(sql, conexion);
                SqlDataReader reader = comando.ExecuteReader();
        ...
        public async Task<IActionResult> confirmar(int ID)
        {
            ...
            switch (message)
            {
                ...
                case "F":
                    ...
                    SqlConnection conexion = Conexion.GetConexion();
                    conexion.Open();
                    string sql = "select p.email, u.email \r\nfrom profesores p, usuarios u, tutoria t \r\nwhere t.id_profesor = p.matricula and t.id_usuario = u.matricula and t.id = @ID;";
                    SqlCommand comando = new SqlCommand(sql, conexion);
                    ...
                    mail.Body = "Su solicitud de tutoria ha sido confirmada, podra ver los detalles en la plataforma unphu virtual";
        ...
        public async Task<IActionResult> rechazar(int ID)
        {
            ...
            switch (message)
            {
                case "A":
                    ...
                    mail.From = new MailAddress("cuposUNPHU@hotmail.com");
                    mail.Subject = "Estado de la solicidud de tutoria";
                    mail.Body = "Su solicitud de tutoria ha sido rechazada debido a que no se alcanzaron los requisitos necesarios";
                    mail.IsBodyHtml = true;
                    SmtpClient smtp = new SmtpClient("smtp.office365.com", 587);
                    smtp.UseDefaultCredentials = false;
                    smtp.EnableSsl = true;
                ...
                case "B":
                    // no porque estatus Inactivo

                    break;

                case "C":
                    // no porque estatus rechazado
                    break;

                case "D":
                    //no porque estatus Activo
                    break;
            }
